...
General notes: - Round lake didn't have a bottom water temperature recorded, but surface water was 20.1 degrees and the lake was 1.5 m so it is likely the bottom water temperature isn't too much different (around 20 deg C), so I used 20.1 as the temperature - mV for sample SR0122L: changed from 1121.6 to 121.6 since likely an error (pH matches perfectly and the surrounding measurements are between 107 and 134) -- but not a problem when looking at the ps object data, since this sample must have been removed for low reads or something - Only one truly replicated sample (LB0208L1 and LB0208L2)
...
copy in just the fasta version of the SILVA 138.1 database (the NR99 that is recommended) because ARB is really hard to install and probably unnecessary Additionally, this includes version uses a combination of SILVA and GTDB
...
done with the following code in Beartooth
...
<!-- Classified 121269 of 123905 sequences (97.87%) -->

assign_tax_zotus.sh uses the zotus_nonchimeric.fa file that was dereplicated a second time and then added this additional number in the centroid rather than OTU number.. I want to use this one.
...
Classified 121265 of 123908 sequences (97.87%)

copy just this taxonomy_16S_zotus.txt onto my computer along with the other esv table file we need

```{bash}
rsync jvonegge@beartooth.arcc.uwyo.edu:/project/seddna/jvonegge/WY_lake_microbes/16S/assign_taxonomy/taxonomy_16S.txt /Users/jordanscheibe/Desktop/WYLakeSedMicrobes

rsync jvonegge@beartooth.arcc.uwyo.edu:/project/seddna/jvonegge/WY_lake_microbes/16S/assign_taxonomy/taxonomy_16S_zotus.txt /Users/jordanscheibe/Desktop/WYLakeSedMicrobes
#this file comes back as empty...

rsync jvonegge@beartooth.arcc.uwyo.edu:/project/seddna/jvonegge/WY_lake_microbes/16S/original_files/otutable.esv /Users/jordanscheibe/Desktop/WYLakeSedMicrobes
```
...
require(stringr)
require(dplyr)
tax_tab_silva <- as.matrix(read.table("taxonomy_16S_zotus.txt", header=F, row.names=1, check.names=F, na.strings="", sep="\t"))

test<-read.table("taxonomy_16S_zotus.txt")

tax_tab_silva <- as.matrix(read.table("/Users/jordanscheibe/OneDrive - University of Wyoming/LakeSedDNA/Data/WYLakeSedMicrobes/Calder_vsearch/16S/original files off Teton/taxonomy_16S_silvav123.txt", header=F, row.names=1, check.names=F, na.strings="", sep="\t"))
