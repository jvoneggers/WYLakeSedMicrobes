...
download the two Silva138 full-length taxonomy and sequence files from mothur's website https://docs.qiime2.org/2023.2/data-resources/#marker-gene-reference-databases

added them to the supercomputer and used code online https://github.com/torognes/vsearch/issues/438 to format into vsearch. 

I did try downloading the Silva 138 fasta file from the website and renaming them (along with using rescript, dada2, qiime/mothur) but as the github vsearch issue talks about, it varies so much that it was nice to have a dataset with the simplified taxonomies into 7 columns. I also could have used the shortened version for the 515F/806R region, but used full length instead. 

```{bash}

rsync /Users/jordanscheibe/Desktop/silva-138-99-tax.qza jvonegge@beartooth.arcc.uwyo.edu:/project/seddna/jvonegge/WY_lake_microbes/16S/reference_database

rsync /Users/jordanscheibe/Desktop/silva-138-99-seqs.qza jvonegge@beartooth.arcc.uwyo.edu:/project/seddna/jvonegge/WY_lake_microbes/16S/reference_database

#unzip these files, and go into the folders and grab out the taxonomy.tsv and the dna-sequences.fasta files and put them in one folder together
...
assign_tax_zotus_silva138_rescript.sh

```{bash}
#!/bin/bash
#SBATCH --job-name assign_taxonomy_zotus_silva138
#SBATCH --mem=110GB
#SBATCH --time=3:00:00
#SBATCH --cpus-per-task=32
#SBATCH --account=microbiome
#SBATCH --output=assign_taxonomy_zotus_silva138_%A.out

cd /project/seddna/jvonegge/WY_lake_microbes/16S/assign_taxonomy/
 
module load arcc/1.0  gcc/12.2.0
module load vsearch/2.22.1

vsearch -sintax zotus_nonchimeric.fa -db /project/seddna/jvonegge/WY_lake_microbes/16S/reference_database/SILVA138_RESCRIPt.fasta -tabbedout taxonomy_16S_zotus_SILVA138_rescript.txt -strand both -sintax_cutoff 0.8 -threads 32
...
copy just this taxonomy_16S_zotus_SILVA138.txt onto my computer along with the other esv table file we need

```{bash}
rsync jvonegge@beartooth.arcc.uwyo.edu:/project/seddna/jvonegge/WY_lake_microbes/16S/assign_taxonomy/taxonomy_16S_zotus_SILVA138_rescript.txt /Users/jordanscheibe/Desktop/WYLakeSedMicrobes
