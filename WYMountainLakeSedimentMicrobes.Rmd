...
```{bash}
salloc --mem=20GB --nodes=1 --cpus-per-task=1 --account=microbiome --time=3:00:00
```
...
pdf("Figures/CommAssemProc_SedDist_29May2023.pdf", height=2.9, width=3.3)
ggplt
dev.off()
...
<!-- DOING THIS WITH ZONE!! -->
<!-- ```{r} -->

<!-- sd_sed$zone<-rep(1,nrow(sd_sed)) -->
<!-- sd_sed[sd_sed$bin_depth%in%c(6,8,10,12),]$zone<-2 -->
<!-- sd_sed[sd_sed$bin_depth%in%c(14,16,18,20,22,24,26),]$zone<-3 -->
<!-- sd_sed[sd_sed$bin_depth==100,]$zone<-4 -->
<!-- #sd_sed$zone<-as.factor(sd_sed$zone) -->

<!-- ord.fit <- envfit(ord_baseR ~  zone + lake_depth + bottom_water_temp,  data = sd_sed,  perm = 1000, na.rm = TRUE) -->
<!-- ord.fit[["vectors"]][["arrows"]][1,1]<-(ord.fit[["vectors"]][["arrows"]][1,1]*(-1)) -->

<!-- pdf("Figures/NMDS_28June2023_noLegend_ZONE.pdf", width=5, height=4.5) -->
<!-- par(mar=c(4, 4, 2, 1), xpd=TRUE) -->
<!-- plot(ord_df$NMDS1, ord_df$NMDS2,pch=16,cex=1.5, ylim=c(-1.5,2.3), yaxt="n", xlim=c(-3,2.3),col=ord_df$zone_col,xlab='', cex.axis=1.2,ylab='') -->
<!-- axis(2,at=c(-1,0,1,2),labels =c(-1,0,1,2), cex.axis=1.2, las=2, col = NA, col.ticks = 1) -->
<!-- title(xlab="NMDS1", line = 2.5, cex.lab=1.2) -->
...
pdf("Figures/RelAbun_TopFam_26Jun2023.pdf", width=16, height=4)
...
ggplt[[2]]<-ggplot(data =Data , aes(x = x, y = fit)) +
        ...
        annotate("text", x=(500*0.75), y=0.7, label="Dev exp = 8.78%",  size=5) +
        ylim(0,0.8)+
        theme_comm  
