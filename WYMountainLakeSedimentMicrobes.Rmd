...
```{bash}
salloc --mem=50GB --nodes=1 --cpus-per-task=1 --account=microbiome --time=4:00:00
```

- Only one truly replicated sample (LB0208L1 and LB0208L2)
-SIMPER analysis between warm and cool lakes
-co-occurance network


Assigning taxonomy was done with the following code in Teton:

Notes on taxonomy and OTU tables 
notes on tax_tab_silva
        a. used zotus.nonchimeric.fa file to make the uniqueOTUs_16S.fa file
            ...
            *JVE this is where that extra number comes in.. I likely didn't need to do this at all since it was dereplicated in the previous script. But at least i know it matches it looks like it de
        b. created in Teton using: vsearch -sintax uniqueOTUs_16S.fa -db /project/microbiome/ref_db/silva_16s_v123.fa -tabbedout taxonomy_16S_v2.txt -strand both -sintax_cutoff 0.8 --threads 32
