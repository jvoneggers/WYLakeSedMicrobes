...
```{bash}
salloc --mem=100GB --nodes=1 --cpus-per-task=1 --account=microbiome --time=4:00:00
```
...
copy in just the fasta version of the SILVA 138.1 database (the NR99 that is reccomended) because ARB is really hard to install and probably unnecessary
...
```{bash}
        ...
        <!-- a. used zotus.nonchimeric.fa file to make the uniqueOTUs_16S.fa file -->
        <!--     cat zotus_nonchimeric.fa | vsearch --derep_fulllength - --output - --sizeout --sizein | perl -pe 's/^(>centroid=)/$1$._/' > uniqueOTUs_16S.fa -->
        <!--     JVE this is where that extra number comes in.. I likely didn't need to do this at all since it was dereplicated in the previous script. But at least i know it matches it looks like it  -->
            
            
vsearch -sintax uniqueOTUs_16S.fa -db /project/microbiome/ref_db/silva_16s_v123.fa -tabbedout taxonomy_16S_v2.txt -strand both -sintax_cutoff 0.8 --threads 32

        c. note the reference database used (silva_16s_v123.fa) for manuscript method writing!!
        d. still need to touch base with Alex on the preferences set to create OTU tables (should be in the Bioinformatics v3 page on Confluence, but need to double check these were all the correct parameters used for manuscript methods)
```
