...
General notes:
- Round lake didn’t have a bottom water temperature recorded, but surface water was 20.1 degrees and the lake was 1.5 m so it is likely the bottom water temperature isn’t too much different (around 20 deg C), so I used 20.1 as the temperature
- mV for sample SR0122L: changed from 1121.6 to 121.6 since likely an error (pH matches perfectly and the surrounding measurements are between 107 and 134) – but not a problem when looking at the ps object data, since this sample must have been removed for low reads or something
- Only one truly replicated sample (LB0208L1 and LB0208L2)

# Part I: Grab files off Beartooth supercomputer and assign taxonomy

copy the bash file used to make the files above! (from Alex Buerkle)
move to folder: /Users/jordanscheibe/OneDrive - University of Wyoming/LakeSedDNA/Data/Sequence Data/Calder_vsearch/16S/original files off Teton
```{bash}
...
copy orginal files from the Micro Project to my folders
but just grab:
1. otutable.esv
2. zotus_nonchimeric.fa
```{bash}
...
copy in just the fasta version of the SILVA 138.1 database (the NR99 that is recommended) because ARB is really hard to install and probably unnecessary
Additionally, this includes version uses a combination of SILVA and GTDB
```{bash}
rsync /Users/jordanscheibe/Desktop/SILVA_138.1_SSURef_NR99_tax_silva.fasta.gz jvonegge@beartooth.arcc.uwyo.edu:/project/seddna/jvonegge/WY_lake_microbes/16S/reference_database
...
Assigning taxonomy was done with the following code in Beartooth

assign_tax.sh is using this uniqueOTUs_16s.fa file


```{bash}
cat zotus_nonchimeric.fa | vsearch --derep_fulllength - --output - --sizeout --sizein | perl -pe 's/^(>centroid=)/$1$._/' > uniqueOTUs_16S.fa
```
...
#SBATCH --output=assign_taxonomy_%A.out

module load arcc/1.0  gcc/12.2.0
module load vsearch/2.22.1

vsearch -sintax uniqueOTUs_16S.fa -db ../reference_database/SILVA_138.1_SSURef_NR99_tax_silva.fasta -tabbedout taxonomy_16S.txt -strand both -sintax_cutoff 0.8 -threads 32

echo "vsearch -sintax uniqueOTUs_16S.fa -db ../reference_database/SILVA_138.1_SSURef_NR99_tax_silva.fasta -tabbedout taxonomy_16S.txt -strand both -sintax_cutoff 0.8 -threads 32"
```

assign_tax_zotus.sh uses the zotus_nonchimeric.fa file that was dereplicated a second time and then added this additional number in the centroid rather than OTU number.. I want to use this one if I can. 

assign_tax_zotus.sh
```{bash}
#!/bin/bash
#SBATCH --job-name assign_taxonomy_zotus
#SBATCH --mem=110GB
#SBATCH --time=3:00:00
#SBATCH --cpus-per-task=32
#SBATCH --account=microbiome
#SBATCH --output=assign_taxonomy_zotus_%A.out

module load arcc/1.0  gcc/12.2.0
module load vsearch/2.22.1

vsearch -sintax zotus_nonchimeric.fa -db ../reference_database/SILVA_138.1_SSURef_NR99_tax_silva.fasta -tabbedout taxonomy_16S_zotus.txt -strand both -sintax_cutoff 0.8 -threads 32

echo "vsearch -sintax zotus_nonchimeric.fa -db ../reference_database/SILVA_138.1_SSURef_NR99_tax_silva.fasta -tabbedout taxonomy_16S_zotus.txt -strand both -sintax_cutoff 0.8 -threads 32"
```
...
require(stringr)
require(dplyr)
tax_tab_silva <- as.matrix(read.table("/Users/jordanscheibe/OneDrive - University of Wyoming/LakeSedDNA/Data/Sequence Data/Calder_vsearch/16S/original files off Teton/taxonomy_16S_silvav123.txt", header=F, row.names=1, check.names=F, na.strings="", sep="\t"))
#wrangle names of tax table to only include centroid ID
#split the rownames by ";" and pull out only the "centroidXXX" info. 
tax_names<-str_split(rownames(tax_tab_silva),pattern = ";", 3, simplify = T)[,1]
#if you have an additional number like 'centroid=21_otu7'and you just want 'centroid=otu7' so that it matches your OTU table and prevents errors when you create a phyloseq object
tax_names<-as.data.frame(cbind(rep("centroid=",length(tax_names)),str_split(tax_names,pattern = "_", 2, simplify = T)[,2]))
tax_names<-paste(tax_names[,1],tax_names[,2],sep="")


#again, this is just to check that the new sample names match up. 
#I created a new dataframe and do the actual check in the lines below.
# All values should equal true if the new names are in the correct spot. 
tax_tab_silva2<-tax_tab_silva

#rename rows with the extracted tax_names

rownames(tax_tab_silva2)<-tax_names
...
table(rownames(tax_tab_silva2) == tax_names)

#extract only the taxonomy column

tax_tab_silva2<-data.frame(tax_tab_silva2[,3])

#convert long string of taxonomy to individual columns. simplify = T 
#is great because it saves it as a matrix with values in indvidual columns. 
taxonomy_names<-data.frame(str_split(tax_tab_silva2$tax_tab_silva2...3.,pattern = ",", 7, simplify = T))

#Bind centroid names and taxonomy
tax_tab_silva2<-cbind(tax_tab_silva2, taxonomy_names)

#rename columns
names(tax_tab_silva2)<-c("Tax", "Kingdom", "Phylum", "Class", "Order", "Family", "Genus", "Species")

#remove original taxonomy string
tax_tab_silva2$Tax=NULL

#replace "" with NA
tax_tab_silva2<-na_if(tax_tab_silva2, "")

#this number of OTUs is not the same as the starting number of OTUs in the ESV table, but deemed ok/close enough (123867)
tax_tab_silva2

#convert to matrix
tax_tab_silva<-as.matrix(tax_tab_silva2)
...
```{r}
#unassigned taxa at the domain level (column 1)
table(is.na(tax_tab_silva[,1]))
#  FALSE   TRUE 
# 121478   2427 
...
#remove Chloroplast in class
chloro<-as.data.frame(which(df1 == "c:Chloroplast", arr.ind = TRUE))[,1]
chloro2<-as.data.frame(which(df1 == "c:Chloroplast", arr.ind = TRUE))

df2<-df1[-chloro,]
nrow(df1)-nrow(df2) #exact number of cholorplast occurrences 1046

#remove Mitochondria in family
mito<-as.data.frame(which(df2 == "f:Mitochondria", arr.ind = TRUE))[,1]
mito2<-as.data.frame(which(df2 == "f:Mitochondria", arr.ind = TRUE))

df3<-df2[-mito,]
nrow(df2)-nrow(df3) #553 - exact number of mitochondrial sequences. 

tax_tab_silva<-as.matrix(df3)
#this should have 119879 taxa (rows) of 7 columns

#now double check there are no lingering ones outside of class and family
t1<-as.data.frame(which(df == "f:Mitochondria", arr.ind = TRUE))
t2<-as.data.frame(which(df == "d:Mitochondria", arr.ind = TRUE))
t3<-as.data.frame(which(df == "p:Mitochondria", arr.ind = TRUE))
t4<-as.data.frame(which(df == "c:Mitochondria", arr.ind = TRUE))
t5<-as.data.frame(which(df == "o:Mitochondria", arr.ind = TRUE))
t6<-as.data.frame(which(df == "g:Mitochondria", arr.ind = TRUE))
t7<-as.data.frame(which(df == "s:Mitochondria", arr.ind = TRUE))
#only found in family
rm(t1)
rm(t2)
rm(t3)
rm(t4)
...
#for chloroplast
t1<-as.data.frame(which(df == "f:Chloroplast", arr.ind = TRUE))
t2<-as.data.frame(which(df == "d:Chloroplast", arr.ind = TRUE))
t3<-as.data.frame(which(df == "p:Chloroplast", arr.ind = TRUE))
t4<-as.data.frame(which(df == "c:Chloroplast", arr.ind = TRUE))
t5<-as.data.frame(which(df == "o:Chloroplast", arr.ind = TRUE))
t6<-as.data.frame(which(df == "g:Chloroplast", arr.ind = TRUE))
t7<-as.data.frame(which(df == "s:Chloroplast", arr.ind = TRUE))
#only found in family
rm(t1)
rm(t2)
rm(t3)
...
newframe<-list()
for(i in 1:7){
        newframe[i]<-as.data.frame(str_split_fixed(tax_tab_silva[,i], ":", n=2)[,2])
}       
taxa_tab<-list.cbind(newframe)

rm(newframe)
rm(i)

#check this worked
Kingdom<-as.data.frame(str_split_fixed(tax_tab_silva[,1], ":", n=2)[,2])
Species<-as.data.frame(str_split_fixed(tax_tab_silva[,7], ":", n=2)[,2])
table(taxa_tab[,1]==Kingdom)
table(taxa_tab[,7]==Species)
rm(Kingdom)
rm(Species)

#both of these are true, now just need to add rownames and column names to the matrix
taxa_tab<-as.data.frame(taxa_tab)
colnames(taxa_tab)<-colnames(tax_tab_silva)
rownames(taxa_tab)<-rownames(tax_tab_silva)
taxa_tab<-as.matrix(taxa_tab)
table(rownames(taxa_tab)==rownames(tax_tab_silva))
table(colnames(taxa_tab)==colnames(tax_tab_silva))
...
## 2. read in OTU table
from folder on Teton, now Beartooth
/project/microbiome/data/seq/psomagen_17sep20_novaseq2/otu/16S/Calder 

```{r}
otu_table<-read.delim("/Users/jordanscheibe/OneDrive - University of Wyoming/LakeSedDNA/Data/Sequence Data/Calder_vsearch/16S/original files off Teton/otutable.esv", header = TRUE, stringsAsFactors = FALSE, quote = "", sep = "\t")
...
H37SG01_1
H42SG15_1
2. remove samples that should not have been included
RL0100L2 #should not have been sequenced
RL0104L #should not have been sequenced
SM0100L #private land, cannot include
SM0104L #private land, cannot include
SV0100L #should not have sequenced first core
SV0104L #should not have sequenced first core
TL0100L2 #biased productivity (near septic)
TL0104L #biased productivity (near septic)
LL0404L #out of order since core starts at 38 cm
LW0100 # duplicated sample, clearly wrong
LWL0104L #sampled poorly 2017, should not have been sequenced
LWL100L #sampled poorly 2017, should not have been sequenced
...
### @count_track
starting, with chloroplast, mitochondria, and NAs included
no water, or samples that shouldn't have been sequenced
11 blanks included here - not the water blank (that was removed)
removed OTUS with no reads
updated 29 aug 2022
```{r}
...
added this september 2nd to calculate: 
total number of reads (blanks removed), kept mitochondria, chloroplast and NAs
also for reads per sample (555 samples without blanks)
...
### @count_track
otu table with blanks and chloroplast/mitochondria/domainNA removed as wel as water and bad samples, blanks kept
updated aug 29, 2022
```{r}
...
at this point you should just have the two data frames in your environment:
1. taxa_tab
2. otu_table (taxonomy filtered, water and bad samples removed, blanks included)
...
sample1	sample2	correlation	reads1	reads2	percent
33_1_0_DNA	33_1_0_DNA	0.41812768	324	1322	25%
35_2_6_DNA	35_2_6_DNA	0.66788087	447	1040	43%
37_1_2_DNA	37_1_2_DNA	0.52398164	104	521	20%
39_1_18_DNA	39_1_18_DNA	0.31364043	164	652	25%
44_1_12_DNA	44_1_12_DNA	0.64393483	161	132952	0%
46_1_2_DNA	46_1_2_DNA	0.69341167	155	168144	0%
BL0126L	BL0126L	0.52864848	348	3941	9%
BL0132L	BL0132L	0.61811876	150	16985	1%
FB0104L	FB0104L	0.34169897	204	283	72%
LB0212L	LB0212L	0.54820268	114	545	21%
LL0104L	LL0104L	0.11559503	129	160	81%
LL0112L	LL0112L	0.53879839	114	287	40%
LS0120L	LS0120L	0.06579572	49014	53193	92%
SR0102L	SR0102L	0.58216221	420	676	62%
SR0104L	SR0104L	0.36381059	159	1577	10%
SR0114L	SR0114L	0.66456196	239	23419	1%
SV0408L	SV0408L	0.5970406	229	235	97%
...
1. mictochondria/chloroplast/NA removed
2. OTUs with less than 10 reads removed
3. samples with less than 100 reads removed
4. technical replicates correlated and summed
5. blanks removed
updated 29 aug 2022
```{r}
sum(colSums(otu_table))
...
create bash file for muscle and fastTree
clustalo_fasttree.sh
```{bash}
#!/bin/bash
...
Call:
lm(formula = meta_sed_26_hills$Species_Richness ~ meta_sed_26_hills$depth)

Residuals:
     Min       1Q   Median       3Q      Max 
-1829.58  -394.87   -16.98   432.42  1375.02 

Coefficients:
                        Estimate Std. Error t value Pr(>|t|)    
(Intercept)             2608.585     47.284  55.168   <2e-16 ***
meta_sed_26_hills$depth  -30.408      3.482  -8.732   <2e-16 ***

Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1

Residual standard error: 554.2 on 427 degrees of freedom
Multiple R-squared:  0.1515,	Adjusted R-squared:  0.1495 
F-statistic: 76.25 on 1 and 427 DF,  p-value: < 2.2e-16


Call:
lm(formula = meta_sed_26_hills$Shannon_Diversity ~ meta_sed_26_hills$depth)

Residuals:
    Min      1Q  Median      3Q     Max 
-877.93 -233.20  -27.67  216.85 1036.11 

Coefficients:
                        Estimate Std. Error t value Pr(>|t|)    
(Intercept)              892.845     27.068  32.985  < 2e-16 ***
meta_sed_26_hills$depth  -14.226      1.993  -7.137 4.13e-12 ***

Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1

Residual standard error: 317.2 on 427 degrees of freedom
Multiple R-squared:  0.1066,	Adjusted R-squared:  0.1045 
F-statistic: 50.93 on 1 and 427 DF,  p-value: 4.134e-12


Call:
lm(formula = meta_sed_26_hills$Simpson_Dominance ~ meta_sed_26_hills$depth)

Residuals:
    Min      1Q  Median      3Q     Max 
-267.24  -86.48  -22.33   68.79  448.47 

Coefficients:
                        Estimate Std. Error t value Pr(>|t|)    
(Intercept)             269.7218    10.4390  25.838  < 2e-16 ***
meta_sed_26_hills$depth  -4.0058     0.7688  -5.211 2.94e-07 ***

Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1

Residual standard error: 122.3 on 427 degrees of freedom
Multiple R-squared:  0.05978,	Adjusted R-squared:  0.05758 
F-statistic: 27.15 on 1 and 427 DF,  p-value: 2.936e-07
...
R-sq.(adj) =   0.15   Deviance explained = 15.2%
GCV = 3.0854e+05  Scale est. = 3.071e+05  n = 429
...
Call:
adonis(formula = adonis_dist ~ depth + water_sample_t_bot + max_lake_depth,      data = sd) 
...
depth                1     9.707  9.7074  27.915 0.05236  0.001 ***
water_sample_t_bot   1     7.267  7.2669  20.897 0.03920  0.001 ***
max_lake_depth       1     3.583  3.5826  10.302 0.01932  0.001 ***
Residuals          474   164.835  0.3478         0.88912           
...
```
Call:
adonis(formula = adonis_dist ~ zone + water_sample_t_bot + max_lake_depth,      data = sd) 
...
Terms added sequentially (first to last)
                    ...
                    Df SumsOfSqs MeanSqs F.Model      R2 Pr(>F)    
zone                 3    13.452  4.4840  13.159 0.07256  0.001 ***
water_sample_t_bot   1     7.384  7.3843  21.670 0.03983  0.001 ***
max_lake_depth       1     3.719  3.7186  10.913 0.02006  0.001 ***
Residuals          472   160.837  0.3408         0.86755           
Total              477   185.392                 1.00000           
...
```
  Dispersal limitation                  Drift  Homogeneous selection Homogenizing dispersal 
          0.1024271291           0.0056665175           0.2045297054           0.0002543793 
    Variable selection 
          0.6871222687 
### Fig 6 - Continuous Assembly Processes
just like the decay relationships, I'm going to only look 26 cm 
then I'll also plot the 26+ ones as a supplementary figure
...
old breaks
 "[0.0162,0.117)" "[0.1167,0.163)" "[0.1632,0.208)" "[0.2080,0.240)" "[0.2399,0.276)" "[0.2759,0.323)" "[0.3234,0.384)"
 [8] "[0.3835,0.454)" "[0.4543,0.540)" "[0.5399,0.722]"
...
11 Jan 2023, update by separate out cores in same lake 
reload 0) from above
```{r}
...
Redox (number of comparisons in each bin)
96	0
75	0.1
705	5
558	10
...
####4) GAMs with 0s for each process
this most recent version breaks out the intra-lake comparisons (so different cores within the same lake (just for the geographic distnace))
```{r}
require(mgcv)
...
```
Call:
lm(formula = sub$env_dist ~ sub$geo_dist_km)

Residuals:
     Min       1Q   Median       3Q      Max 
-0.29498 -0.11251 -0.01654  0.10730  0.39676 

Coefficients:
                  Estimate Std. Error t value Pr(>|t|)    
(Intercept)      3.255e-01  9.463e-03   34.40   <2e-16 ***
sub$geo_dist_km -5.871e-05  3.394e-05   -1.73   0.0842 .  

Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1

Residual standard error: 0.1522 on 662 degrees of freedom
Multiple R-squared:  0.004498,	Adjusted R-squared:  0.002994 
F-statistic: 2.991 on 1 and 662 DF,  p-value: 0.08418
...
Mantel statistic r: 0.2314 
      Significance: 0.001 

Upper quantiles of permutations (null model):
    90%     95%   97.5%     99% 
0.00874 0.01089 0.01331 0.01641 
Permutation: free
Number of permutations: 999
...
Mantel statistic r: 0.1938 
      Significance: 0.001 

Upper quantiles of permutations (null model):
   90%    95%  97.5%    99% 
0.0091 0.0119 0.0146 0.0191 
Permutation: free
Number of permutations: 999
...
Mantel statistic r: 0.2283 
      Significance: 0.001 

Upper quantiles of permutations (null model):
   90%    95%  97.5%    99% 
0.0088 0.0108 0.0130 0.0161 
Permutation: free
Number of permutations: 999
...
Mantel statistic r:  0.19 
      Significance: 0.001 

Upper quantiles of permutations (null model):
   90%    95%  97.5%    99% 
0.0109 0.0147 0.0176 0.0209 
Permutation: free
Number of permutations: 999
...
```
(shotgun_env) [jvonegge@blog1 maketree_clustalo]$ salloc --mem=120GB --nodes=1 --cpus-per-task=6 --account=microbiome --time=1:00:00
salloc: Pending job allocation 6104675
salloc: job 6104675 queued and waiting for resources
salloc: job 6104675 has been allocated resources
salloc: Granted job allocation 6104675
salloc: Waiting for resource configuration
salloc: Nodes m221 are ready for job
[jvonegge@m221 maketree_clustalo]$ module load arcc/1.0 miniconda3/4.12.0
[jvonegge@m221 maketree_clustalo]$ conda activate shotgun_env
cd /project/seddna/jvonegge/WY_lake_microbes/16S/phylogenetic_tree/tree_13Jan2023/maketree_clustalo
(shotgun_env) [jvonegge@m221 maketree_clustalo]$ cd /project/seddna/jvonegge/WY_lake_microbes/16S/phylogenetic_tree/tree_13Jan2023/maketree_clustalo
(shotgun_env) [jvonegge@m221 maketree_clustalo]$ ls
ESVs_with10ormorereads_forBeartooth_16Jan2023.fasta
(shotgun_env) [jvonegge@m221 maketree_clustalo]$ clustalo -i ESVs_with10ormorereads_forBeartooth_16Jan2023.fasta -o tempfile_16Jan2023_muscled.fa -v --threads=6
Using 6 threads
Read 28316 sequences (type: DNA) from ESVs_with10ormorereads_forBeartooth_16Jan2023.fasta
Using 218 seeds (chosen with constant stride from length sorted seqs) for mBed (from a total of 28316 sequences)
Calculating pairwise ktuple-distances...
Ktuple-distance calculation progress done. CPU time: 494.62u 0.12s 00:08:14.74 Elapsed: 00:01:26
mBed created 448 cluster/s (with a minimum of 1 and a soft maximum of 100 sequences each)
Distance calculation within sub-clusters done. CPU time: 94.02u 0.03s 00:01:34.05 Elapsed: 00:00:17
Guide-tree computation (mBed) done.
Progressive alignment progress done. CPU time: 348.22u 1.90s 00:05:50.12 Elapsed: 00:04:25
Alignment written to tempfile_16Jan2023_muscled.fa


(shotgun_env) [jvonegge@m221 maketree_clustalo]$ FastTree -nt tempfile_16Jan2023_muscled.fa > ESVs_with10ormorereads_output_16Jan2023_muscled.nwk
ML-NNI round 11: LogLk = -911140.600 NNIs 337 max delta 10.59 Time 656.25 (final)
Optimize all lengths: LogLk = -911087.046 Time 669.69  
...
#### 2) GAMS without 0
doesn't include comparisons within the same lake
8Feb2023 - the labeling for geographic distance is off (double check if going to put in supplementary figures)
...
Mantel statistic r: 0.246 
      Significance: 0.001 

Upper quantiles of permutations (null model):
    90%     95%   97.5%     99% 
0.00917 0.01263 0.01589 0.01806 
Permutation: free
Number of permutations: 999
...
Mantel statistic r: 0.2047 
      Significance: 0.001 

Upper quantiles of permutations (null model):
   90%    95%  97.5%    99% 
0.0124 0.0165 0.0197 0.0241 
Permutation: free
Number of permutations: 999
...
Mantel statistic r: 0.2014 
      Significance: 0.001 

Upper quantiles of permutations (null model):
   90%    95%  97.5%    99% 
0.0120 0.0157 0.0193 0.0256 
Permutation: free
Number of permutations: 999
